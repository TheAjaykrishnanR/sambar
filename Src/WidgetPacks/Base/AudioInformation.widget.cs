public class AudioInformation : Widget
{
	...
	public AudioInformation(WidgetEnv ENV) : base(ENV)
	{
		...
		wnd = Sambar.api.CreateWidgetWindow(425, -210, 400, 50, centerOffset: true);
		wnd.Background = new SolidColorBrush(System.Windows.Media.Colors.Transparent);
		wnd.Content = border;
		wnd.Show();

		Sambar.api.MEDIA_INFO_EVENT += MediaInfoEventHandler;
		Sambar.api.MEDIA_STOPPED_EVENT += MediaStoppedEventHandler;
	}

	void MediaInfoEventHandler(MediaInfo info)
	{
		wnd?.Dispatcher.Invoke(() => textBlock.Text = info.Title);
	}

	void MediaStoppedEventHandler()
	{
